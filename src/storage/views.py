...
def index_view(request):
    ...
    path = f"{path.rstrip('/')}/" if path else "" # Обязательный / в конце
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request, f"Файл {file_name} успешно загружкен.")
                return JsonResponse({
...
def create_folder_view(request):
    if request.method == "POST":
        ...
        current_path = request.POST.get("current_path", "")
        create_and_go = request.POST.get("create_and_go", False)
        user_id = request.user.id
        ...
        try:
            ...
            return redirect("home")  # Или на страницу с файлом
